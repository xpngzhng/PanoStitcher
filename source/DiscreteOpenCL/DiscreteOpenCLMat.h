...
struct HostMem
{
    ...
    void create(int rows_, int cols_, int type_)
    {
        ...
        if (rows != rows_ || cols != cols_ || type != (type_& CV_MAT_TYPE_MASK))
        {
            ...
            int err = 0;
            data = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, rows * step, data, &err);
            sdata.reset(data, clReleaseMemObject);
            if (err)
            {
                clear();
            }
    ...
    void copyTo(HostMem& other) const
    {
        ...
        clEnqueueCopyBuffer(ocl->queue, data, other.data, 0, 0, step * rows, 0, 0, 0);
    ...
    cv::Mat mapToHost()
    {
        CV_Assert(mapped == 0);
        int err = 0;
        mappedPtr = clEnqueueMapBuffer(ocl->queue, data, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, step * rows, 0, 0, 0, &err);
        SAMPLE_CHECK_ERRORS(err);
    ...
    void unmapFromHost(const cv::Mat& mat)
    {
        ...
        int err = 0;
        err = clEnqueueUnmapMemObject(ocl->queue, data, mappedPtr, 0, 0, 0);
        SAMPLE_CHECK_ERRORS(err);
        err = clFinish(ocl->queue);
        SAMPLE_CHECK_ERRORS(err);
        mapped = 0;
...
struct GpuMat
{
    ...
    void create(int rows_, int cols_, int type_)
    {
        ...
        if (rows != rows_ || cols != cols_ || type != (type_& CV_MAT_TYPE_MASK))
        {
            ...
            step = elemSize1 * channels * cols;
            int err = 0;
            data = clCreateBuffer(ctx, CL_MEM_READ_WRITE, rows * step, data, &err);
            sdata.reset(data, clReleaseMemObject);
            if (err)
            {
                clear();
    ...
    void copyTo(GpuMat& other) const
    {
        other.create(size(), type);
        int err = 0;
        err = clEnqueueCopyBuffer(ocl->queue, data, other.data, 0, 0, step * rows, 0, 0, 0);
        SAMPLE_CHECK_ERRORS(err);
        err = clFinish(ocl->queue);
        SAMPLE_CHECK_ERRORS(err);
    }
    ...
    void upload(const HostMem& mem)
    {
        CV_Assert(mem.data);

        create(mem.rows, mem.cols, mem.type);
        int err = 0;
        err = clEnqueueCopyBuffer(ocl->queue, mem.data, data, 0, 0, step * rows, 0, 0, 0);
        SAMPLE_CHECK_ERRORS(err);
        err = clFinish(ocl->queue);
        SAMPLE_CHECK_ERRORS(err);
    }

    void download(HostMem& mem) const
    {
        CV_Assert(data);

        mem.create(rows, cols, type);
        int err = 0;
        err = clEnqueueCopyBuffer(ocl->queue, data, mem.data, 0, 0, step * rows, 0, 0, 0);
        SAMPLE_CHECK_ERRORS(err);
        err = clFinish(ocl->queue);
        SAMPLE_CHECK_ERRORS(err);
    }
