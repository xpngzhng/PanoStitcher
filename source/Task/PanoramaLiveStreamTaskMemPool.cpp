...
struct PanoramaLiveStreamTask::Impl
{
    ...
    std::unique_ptr<std::vector<SharedAudioVideoFramePool> > videoFramePools;
    ...
    SharedAudioVideoFramePool audioFramePool;
    int audioSampleRate;
...
bool PanoramaLiveStreamTask::Impl::openVideoDevices(const std::vector<avp::Device>& devices, int width, int height, int frameRate, std::vector<int>& success)
{
    ...
    //syncedFramesBufferForShow.setMaxSize(4);
    //syncedFramesBufferForProc.setMaxSize(4);

    videoFramePools.reset(new std::vector<SharedAudioVideoFramePool>(numVideos));
    for (int i = 0; i < numVideos; i++)
        (*videoFramePools)[i].initAsVideoFramePool(pixelType, videoFrameSize.width, videoFrameSize.height);
...
bool PanoramaLiveStreamTask::Impl::openAudioDevice(const avp::Device& device, int sampleRate)
{
    ...
    int sampleType = audioReader.getAudioSampleType();
    int numChannels = audioReader.getAudioNumChannels();
    int channelLayout = audioReader.getAudioChannelLayout();
    int numSamples = audioReader.getAudioNumSamples();
    if (!numSamples)
    {
        avp::AudioVideoFrame frame;
        while (true)
        {
            audioReader.read(frame);
            if (frame.mediaType == avp::AUDIO)
                break;
        }
        numSamples = frame.numSamples;
    }
    audioFramePool.initAsAudioFramePool(sampleType, numChannels, channelLayout, numSamples);
...
void PanoramaLiveStreamTask::Impl::videoSource(int index)
{
    ...
    SharedAudioVideoFramePool& pool = (*videoFramePools)[index];
    avp::AudioVideoReader& reader = videoReaders[index];

    long long int count = 0, beginCheckCount = roundedVideoFrameRate * 5;
    ztool::Timer timer;
    //avp::AudioVideoFrame frame;
    avp::AudioVideoFrame shallowFrame, dummyAudioFrame;
    avp::SharedAudioVideoFrame deepFrame;
    int mediaType;
    bool ok;
    while (true)
    {
        ok = pool.get(deepFrame);
        if (!ok || !deepFrame.data)
        {
            ptlprintf("Error in %s [%8x], cannot get frame from memory pool\n", __FUNCTION__, id);
        ...
        shallowFrame = deepFrame;
        ok = reader.readTo(dummyAudioFrame, shallowFrame, mediaType);
        deepFrame.timeStamp = shallowFrame.timeStamp;
...
void PanoramaLiveStreamTask::Impl::videoSink()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            //printf("size for show = %d, for proc = %d\n", syncedFramesBufferForShow.size(), syncedFramesBufferForProc.size());
...
void PanoramaLiveStreamTask::Impl::audioSource()
{
    ...
    ztool::Timer timer;
    //avp::AudioVideoFrame frame;
    avp::AudioVideoFrame shallowFrame, dummyVideoFrame;
    avp::SharedAudioVideoFrame deepFrame;
    int mediaType;
    bool ok;
    while (true)
    {
        ...
        ok = audioFramePool.get(deepFrame);
        if (!ok || !deepFrame.data)
        {
            ptlprintf("Error in %s [%8x], cannot get frame from memory pool\n", __FUNCTION__, id);
        ...
        shallowFrame = deepFrame;
        ok = audioReader.readTo(shallowFrame, dummyVideoFrame, mediaType);
        deepFrame.timeStamp = shallowFrame.timeStamp;
        if (!ok || mediaType != avp::AUDIO)
        {
        ...
        if (streamOpenSuccess || fileConfigSet)
        {
            ...
            if (streamOpenSuccess)
                procFrameBufferForSend.push(deepFrame);
            if (fileConfigSet)
                procFrameBufferForSave.push(deepFrame);
        }
    }
