...
pyrDown_(const cv::Mat& _src, cv::Mat& _dst, void* aux1, void* aux2, int horiBorderType, int vertBorderType)
{
    const int PD_SZ = 5;
    ...
    //std::vector<int> _buf(bufstep*PD_SZ + 16);
    //int* buf = cv::alignPtr((int*)_buf.data(), 16);
    int* buf = cv::alignPtr((int*)aux1, 16);
    int tabL[CV_CN_MAX*(PD_SZ + 2)], tabR[CV_CN_MAX*(PD_SZ + 2)];
    //cv::AutoBuffer<int> _tabM(dsize.width*cn);
    //std::vector<int> _tabM(dsize.width*cn);
    //int* tabM = _tabM.data();
    int* tabM = (int*)aux2;
    int* rows[PD_SZ];
...
typedef void(*PyrFuncTo32S)(const cv::Mat&, cv::Mat&, void*, void*, int, int);

void pyramidDownTo32S(const cv::Mat& src, cv::Mat& dst, void* aux1, void* aux2, 
...
pyrDown_(const cv::Mat& _src, cv::Mat& _dst, void* aux1, void* aux2, int horiBorderType, int vertBorderType)
{
    ...
    //WT* buf = cv::alignPtr((WT*)_buf.data(), 16);
    WT* buf = cv::alignPtr((WT*)aux1, 16);
    int tabL[CV_CN_MAX*(PD_SZ + 2)], tabR[CV_CN_MAX*(PD_SZ + 2)];
    //cv::AutoBuffer<int> _tabM(dsize.width*cn);
    //std::vector<int> _tabM(dsize.width*cn);
    //int* tabM = _tabM.data();
...
template<class CastOp, class VecOp> void
pyrUp_(const cv::Mat& _src, cv::Mat& _dst, void* aux1, void* aux2, int horiBorderType, int vertBorderType)
{
    const int PU_SZ = 3;
    typedef typename CastOp::type1 WT;
    typedef typename CastOp::rtype T;
    ...
    //std::vector<int> _dtab(ssize.width*cn);
    //int* dtab = _dtab.data();
    int* dtab = (int*)aux2;
    WT* rows[PU_SZ];
    CastOp castOp;
...
typedef void(*PyrFunc)(const cv::Mat&, cv::Mat&, void*, void*, int, int);

void pyramidDown(const cv::Mat& src, cv::Mat& dst, void* aux1, void* aux2, const cv::Size& _dsz,
...
void pyramidUp(const cv::Mat& src, cv::Mat& dst, void* aux1, void* aux2, const cv::Size& _dsz,
    int horiBorderType, int vertBorderType)
{
    CV_Assert(horiBorderType == cv::BORDER_DEFAULT || horiBorderType == cv::BORDER_WRAP);
    CV_Assert(vertBorderType == cv::BORDER_DEFAULT || vertBorderType == cv::BORDER_WRAP);
